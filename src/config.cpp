...
bool ParsedBool(const cameraunlock::IniReader& ini, const char* section, const char* key,
                bool& out) {
...
// Every (section, key) the file actually writes, folded, filled in by the scan
// below before any reader runs. It is what tells a key the user blanked from a
// key they never wrote, and both are needed: `LimitX=`, `WorldSpaceYaw=` and
// `LimitX=;metres` all trim to nothing, and every reader answered them by
// keeping the value the key already held with NOTHING in the log - the same
// silent ignore this file's own comment above says was closed for unparseable
// values. UdpPort already reported it, which left the file inconsistent about
// one input shape.
//
// Answered from our own parse rather than by probing GetPrivateProfileString
// with two different fallbacks. That trick works, but only because a
// present-but-empty key happens to leave the thread's last error at
// ERROR_SUCCESS where an absent one does not, and nothing documents that. A
// silent change there would put this file back exactly where it started, with
// twenty lines of code asserting otherwise.
std::vector<std::pair<std::string, std::string>> g_presentKeys;

bool KeyIsPresent(const char* section, const char* key) {
    const std::pair<std::string, std::string> entry(Folded(section), Folded(key));
    return std::find(g_presentKeys.begin(), g_presentKeys.end(), entry) != g_presentKeys.end();
}

// One wording for every reader, so a blanked value reads the same whichever key
// it was.
void ReportEmptyValue(const char* section, const char* key) {
    if (!KeyIsPresent(section, key)) return;
    Log::Line("[config] %s is set but has no value, so it keeps its default. A trailing ; "
              "comment is part of the value here - put comments on their own line above the "
              "key.", key);
}

bool ReadFlag(const cameraunlock::IniReader& ini, const char* section, const char* key,
              bool current) {
    const std::string text = ini.ReadString(section, key, "");
    if (text.empty()) {
        ReportEmptyValue(section, key);
        return current;
    }
...
float ReadFloatValue(const cameraunlock::IniReader& ini, const char* section, const char* key,
...
float ReadSmoothing(const cameraunlock::IniReader& ini, const char* section, const char* key,
                    float current, float shipped_default) {
...
void WarnRetiredSmoothingKey(const cameraunlock::IniReader& ini, const char* section,
                             const char* key) {
...
void WarnAboutDuplicateEntries(const std::string& path) {
    ...
    std::string line;
    bool first = true;
    bool deadSection = false;
    while (std::getline(file, line)) {
        // A UTF-8 BOM is three ordinary characters to GetPrivateProfileString,
        // which only knows the UTF-16 one. Notepad and PowerShell 5.1's
        // `Out-File -Encoding utf8` both write one.
        //
        // It is only DESTRUCTIVE when it lands on a section header: the line then
        // reads as the three BOM bytes followed by [Network], which is not a
        // header, so every key in that section becomes invisible - and the first
        // section in the shipped file is the UDP port, so the symptom is a
        // tracker sending to a port the mod never bound, with nothing at all in
        // the log to say so. The shipped file opens with a
        // COMMENT, which is where a BOM lands for anyone who just opens and
        // saves, and there it changes nothing. Warning about that healthy file
        // every launch would send the player off to rewrite a working config
        // while whatever actually stopped tracking went unlooked-for, which is
        // the triage cost this line exists to remove, pointed the wrong way. So
        // the BOM is stripped for this scan either way, and reported only when
        // it is sitting on a header.
        bool bom = false;
        if (first) {
            first = false;
            bom = line.size() >= 3 && static_cast<unsigned char>(line[0]) == 0xEF &&
        ...
        if (bom && !text.empty() && text[0] == '[') {
            // The heading is named rather than described. It is the one the
            // player has to go and look at, and naming it is also what lets the
            // suite tell this line apart from the case where a BOM landed on a
            // comment and cost nothing.
            const std::size_t close = text.find(']');
            const std::string name =
                close == std::string::npos ? text.substr(1) : text.substr(1, close - 1);
            Log::Line("[config] %s starts with a UTF-8 byte order mark sitting on the [%s] "
                      "heading, so that heading is not recognised and every key under it is "
                      "ignored. Re-save it as ANSI, or as UTF-8 without a BOM.",
                      kIniName, Trimmed(name).c_str());
        }
        if (text.empty() || text[0] == ';') continue;

        if (text[0] == '[') {
            // A missing ']' does NOT make this a non-heading. The profile API
            // accepts "[Position" and reads the keys under it, so skipping the
            // line here left `section` pointing at the PREVIOUS heading: keys
            // under it were recorded against the wrong section, which both hid
            // them from the presence map and produced a duplicate-key warning
            // for two keys the reader sees in different sections.
            const std::size_t close = text.find(']');
            const std::string name =
                Trimmed(close == std::string::npos ? text.substr(1) : text.substr(1, close - 1));
            section = Folded(name);
            if (section.empty()) continue;
            if (std::find(sections.begin(), sections.end(), section) != sections.end()) {
                // Everything under a repeat of a heading is dead to the reader,
                // which is what this line says. It also must not be recorded as
                // present: a key that lives only here reads back empty, and
                // reporting that as "set but has no value" sends the player
                // looking for a trailing comment on a line that has a perfectly
                // good value on it.
                deadSection = true;
        ...
        // Presence for the readers, recorded here because this is the only place
        // that parses the whole file. A key written twice is recorded once,
        // which is right: the readers only ever see the first. A key under a
        // repeated heading is not recorded at all, because the reader cannot see
        // it either.
        if (!deadSection &&
            std::find(g_presentKeys.begin(), g_presentKeys.end(), entry) == g_presentKeys.end()) {
            g_presentKeys.push_back(entry);
        }
...
void ReadNetworkSection(const cameraunlock::IniReader& ini, Config& out) {
    ...
    // Only where the parse STOPPED matters here; the value itself has already
    // been refused by NormalizeUdpPort.
    char* end = nullptr;
...
void ReadGeneralSection(const cameraunlock::IniReader& ini, Config& out) {
    ...
    out.compensate_world_markers =
        ReadFlag(ini, "General", "CompensateWorldMarkers", out.compensate_world_markers);
}
...
void ReadHotkeysSection(const cameraunlock::IniReader& ini, Config& out) {
    ...
    WarnAboutCollidingHotkeys(nav, std::size(nav));
    WarnAboutCollidingHotkeys(chord, std::size(chord));
}

void ReadRotationSection(const cameraunlock::IniReader& ini, Config& out) {
    out.local_smoothing = ReadSmoothing(ini, "Rotation", "LocalSmoothing",
                                        out.local_smoothing, kDefaultLocalSmoothing);
    out.remote_smoothing = ReadSmoothing(ini, "Rotation", "RemoteSmoothing",
                                         out.remote_smoothing, kDefaultRemoteSmoothing);
