...
public:
    ...
    // rotation (DEGREES, engine sign convention) and position offset (world
    // units, camera-local forward/left/up). `active` is the mod's own gate -
    // the toggle and the gameplay check - passed in rather than mirrored here
    // so the two cannot drift.
    //
    // Degrees rather than radians, because degrees are what the protocol
    // carries and what the log prints. The conversion to radians happens where
...
private:
    ...
    // Degrees, engine sign convention.
    PublishedTriple m_rotation;
    // World units, camera-local forward / left / up.
