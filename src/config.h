...
struct Config {
    ...
    // Head yaw about world up rather than about the camera's own up axis. Indy
    // is a person standing on the ground, so the horizon means something and
    // both positions are defensible - hence the toggle. World up is the default
    // because it is what keeps a glance left level while the player is looking
    // up or down a stairwell, which this game does a great deal of.
    ...
    // Whether world-anchored interface elements - interaction prompts, in-world
    // markers - and the reticle are moved to follow the head-tracked view.
    //
    // On, because without it those elements sit still while the world sweeps
    // past underneath them and the reticle stops marking where shots land. It
    // remains a switch because it hooks four more engine functions than the
    // camera alone does; turning it off leaves the interface exactly as the game
    // draws it and costs nothing but the drift.
    bool compensate_world_markers = true;
