...
// Opens HeadTracking.log beside TheGreatCircle.exe. The core truncates on open
// and keeps the previous launch as HeadTracking.prev.log, so a session never
// appends to an older one.
void OpenLogFile();

// The process-wide log lives in cameraunlock-core. Alias it so call sites read
