...
void TrackerFeed::Update(bool active) {
    ...
    if (m_session.GetPositionOffset(x, y, z)) {
        ...
        // a head leaning IN, while this engine's basis rows are forward / left /
        // up, so forward is -z, negated exactly once, here, after the processor
        // has applied its asymmetric forward/back clamp. x passes through.
        //
        // The three signs and the axis mapping are the id Tech 5 ancestor's,
        // unchanged: wolfenstein-the-new-order-headtracking negated x and roll
        // here until both were reported wrong in game on 2026-09-06 and dropped.
        // Re-deriving them from handedness would throw that measurement away and
        // put a coin flip per axis in front of the player. The scale is NOT the
        // ancestor's - see kUnitsPerMetre.
