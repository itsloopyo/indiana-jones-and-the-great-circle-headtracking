...
function Get-LegacyCMakeVersion {
    param([string]$Path)
    $line = Select-String -LiteralPath $Path -List -Pattern 'project\(GreatCircleHeadTracking\s+VERSION\s+([0-9]+\.[0-9]+\.[0-9]+)'
    if (-not $line) { return '' }
...
try {
    ...
    Check 'Get-ModProject names the ASI stem the packager and deploy both stage' `
        ($project.Name -eq 'GreatCircleHeadTracking') "got '$($project.Name)'"

    Check 'Get-ModProject uses the games.json id install.cmd resolves the game with' `
        ($project.Id -eq 'indiana-jones-and-the-great-circle') "got '$($project.Id)'"
    ...
    Check 'Get-ModProject derives the ASI filename from the mod name' `
        ($project.AsiFileName -eq 'GreatCircleHeadTracking.asi') "got '$($project.AsiFileName)'"

    Check 'Get-ModProject points at the Release build output CMake produces' `
        ($project.BuildOutputPath -eq (Join-Path $project.Root 'build/Release/GreatCircleHeadTracking.asi')) `
    ...
    Set-Content -LiteralPath $cmake -Value @(
        ...
        'project(GreatCircleHeadTracking VERSION 1.4.2 LANGUAGES CXX)'
        'add_library(GreatCircleHeadTracking SHARED src/dllmain.cpp)'
    )
    ...
    Set-Content -LiteralPath $twoPart -Value 'project(GreatCircleHeadTracking VERSION 1.4 LANGUAGES CXX)'
    ...
    Set-Content -LiteralPath $noVersion -Value 'project(GreatCircleHeadTracking LANGUAGES CXX)'
    ...
    Set-Content -LiteralPath $installCmd -Value @(
        ...
        'set "MOD_NAME=GreatCircleHeadTracking"'
    )

    $proxy = Get-AsiLoaderProxyName -InstallCmdPath $installCmd
    Check 'Get-AsiLoaderProxyName reads the proxy install.cmd deploys' ($proxy -eq 'winmm.dll') "got '$proxy'"
