...
void TestAMissingConfigLeavesEveryValueAlone() {
    ...
    Check(c.udp_port == 1234 && c.limit_x == 0.11f && !c.world_space_yaw,
...
void TestSmoothingIsFiniteAndInRange() {
    ...
    CheckNear(nan_.local_smoothing, gc_ht::kDefaultLocalSmoothing, 1e-6f,
    ...
    CheckNear(nan_.remote_smoothing, gc_ht::kDefaultRemoteSmoothing, 1e-6f,
    ...
    CheckNear(inf.local_smoothing, gc_ht::kDefaultLocalSmoothing, 1e-6f,
...
void TestPositionLimitsAreClamped() {
    // Seeded away from the defaults so the non-finite case below can tell a
    // substitution from a reader that never touched the key. With a default
    // seed, `current` IS the default, so the check passed either way.
    Config seed;
    seed.limit_z_back = 0.07f;
    const Config c = Load("limits",
                          "[Position]\nLimitX=4.0\nLimitY=-1\nLimitZ=0.25\nLimitZBack=inf\n", seed);
    CheckNear(c.limit_x, gc_ht::kMaxPositionLimit, 1e-6f,
    ...
    // Seeded so the fallback cannot be a hard-coded default wearing `current`'s
    // clothes: with a default seed those are the same number. What separates a
    // substitution from a key the reader never touched is the log line, which is
    // asserted in full below - key, offending value and the number used instead -
    // so a break confined to the position limits cannot hide behind the generic
    // fragment the log check used to look for.
    CheckNear(c.limit_z_back, 0.07f, 1e-6f,
              "a non-finite limit falls back to the value the key already held");
}
...
// The same-section duplicate. GetPrivateProfileString answers with the first of
// them, so the later one is silently dead and has to be reported. Only the
// duplicated SECTION case was covered, so the key scanner could have been
// deleted with the suite still green.
void TestOnlyTheFirstOfADuplicatedKeyIsRead() {
    const Config c = Load("dup_key",
                          "[Position]\nLimitZ=0.25\nLimitZ=0.45\n");
    CheckNear(c.limit_z, 0.25f, 1e-6f, "the second LimitZ in one section is not read");
}

// Two actions on one key means one of them can never fire, which the loader
// cannot fix and therefore has to report. Nothing exercised this, so the whole
// collision check could have been removed without a failing check.
void TestCollidingHotkeysAreReported() {
    const Config c = Load("key_clash",
                          "[Hotkeys]\nToggleKey=0x23\nCycleModeKey=0x23\n");
    Check(c.toggle_key == 0x23 && c.cycle_mode_key == 0x23,
          "a colliding binding is still taken as written; the log is what carries the warning");
}

// Present but empty. Every reader but UdpPort answered these by keeping the
// shipped default with nothing at all in the log, which is the silent ignore
// this suite exists to stop.
void TestAnEmptyValueKeepsTheDefaultAndSaysSo() {
    Config seed;
    seed.limit_z = 0.33f;
    seed.world_space_yaw = false;
    const Config c = Load("empty_value",
                          "[General]\nWorldSpaceYaw=\n\n[Position]\nLimitZ=\n", seed);
    CheckNear(c.limit_z, 0.33f, 1e-6f, "a blanked float keeps the value it had");
    Check(!c.world_space_yaw, "a blanked bool keeps the value it had");
}

// A UTF-8 BOM is three ordinary characters to GetPrivateProfileString, so a
// heading it lands on stops being a heading and every key under it goes unread.
// Here that is the UDP port, and the symptom is a tracker sending to a port the
// mod never bound.
void TestAUtf8BomOnAHeadingIsReported() {
    Config seed;
    seed.udp_port = 8084;
    const Config c = Load("utf8_bom",
                          "\xEF\xBB\xBF[Network]\nUdpPort=4343\n", seed);
    Check(c.udp_port == 8084, "the port under a BOM-hidden section is not read");
}

// The same three bytes on a COMMENT line cost nothing: the parser ignores that
// line either way and every heading below it is still found. That is where a BOM
// lands for anyone who opens the shipped file and saves it, so warning about it
// would send a player off to rewrite a working config while whatever actually
// stopped tracking went unlooked-for.
void TestAUtf8BomOnACommentIsHarmlessAndUnreported() {
    Config seed;
    seed.limit_z = 0.33f;
    const Config c = Load("utf8_bom_comment",
                          "\xEF\xBB\xBF; my notes\n[Position]\nLimitZ=0.45\n", seed);
    CheckNear(c.limit_z, 0.45f, 1e-6f, "a BOM on a comment line leaves the file fully readable");
}
...
// For a line that must be emitted for one case and NOT for another. A "says" and
// a "says not" pair cannot express that on its own: the fragment is present
// either way, and only the count separates a warning that fired once from one
// that fired for every case that reached it.
void CheckLogSaysTimes(const std::string& log, const char* fragment, std::size_t times,
                       const char* what) {
    std::size_t seen = 0;
    for (std::size_t at = log.find(fragment); at != std::string::npos;
         at = log.find(fragment, at + 1)) {
        ++seen;
    }
    if (seen != times) {
        std::printf("  [FAIL] %s (found %zu, expected %zu)\n", what, seen, times);
        ++checks::g_failures;
        return;
    }
    Check(true, what);
...
void TestEveryRefusalIsInTheLog(const std::string& log) {
    ...
    CheckLogSays(log, "LimitZBack=inf is out of range or not finite; using 0.0700",
                 "the non-finite limit names the key, the value and the number used instead");
    ...
    CheckLogSays(log, "sets LimitZ more than once", "a duplicated key is logged");
    CheckLogSays(log, "are both 0x23", "two actions sharing one key is logged");
    CheckLogSays(log, "is set but has no value", "a present but empty value is logged");
    // The other half of that check, and the one a plausible edit breaks. If
    // KeyIsPresent ever answered true for a key the file does not contain, every
    // partial INI above would log a line per unwritten key and the assertion
    // above would still pass. EnableOnStartup is absent from every partial case
    // and set with a value in the shipped default, so it is the clean witness.
    CheckLogSaysNot(log, "EnableOnStartup is set but has no value",
                    "a key that is absent is not reported as one that is empty");
    CheckLogSays(log, "byte order mark sitting on the [Network] heading",
                 "a byte order mark that hides a heading is logged, and names it");
    // Exactly once across the whole run. Two cases feed a BOM in - one on a
    // heading, one on a comment - and only the second must stay silent, so a
    // count is the assertion. A "says not" on the Position spelling would pass
    // even if the comment case did warn, because it would name whatever text it
    // found rather than a section.
    CheckLogSaysTimes(log, "byte order mark sitting on", 1,
                      "a byte order mark on a comment line costs nothing and is not reported");
...
int main() {
    ...
    TestOnlyTheFirstOfADuplicatedKeyIsRead();
    TestCollidingHotkeysAreReported();
    TestAnEmptyValueKeepsTheDefaultAndSaysSo();
    TestAUtf8BomOnAHeadingIsReported();
    TestAUtf8BomOnACommentIsHarmlessAndUnreported();
