...
// negates on the way from the tracker's convention to the engine's.
//
...
using gc_ht::idtech::Mat3;
using gc_ht::idtech::Dot;
...
void TestIdentity() {
    const Mat3 out = gc_ht::idtech::RotateBasisLocal(Level(), 0.0f, 0.0f, 0.0f);
...
void TestYawTurnsLeft() {
    const Mat3 out = gc_ht::idtech::RotateBasisLocal(Level(), 20.0f * kDeg, 0.0f, 0.0f);
...
void TestPitchRaises() {
    const Mat3 out = gc_ht::idtech::RotateBasisLocal(Level(), 0.0f, 20.0f * kDeg, 0.0f);
...
void TestRollTiltsUpTowardLeft() {
    const Mat3 out = gc_ht::idtech::RotateBasisLocal(Level(), 0.0f, 0.0f, 20.0f * kDeg);
...
void TestCompositionOrder() {
    ...
    const Mat3 combined = gc_ht::idtech::RotateBasisLocal(Level(), y, p, r);
    for (int i = 0; i < 9; ++i) {
    ...
    const Mat3 steep =
        gc_ht::idtech::RotateBasisLocal(Level(), 40.0f * kDeg, 40.0f * kDeg, 35.0f * kDeg);
...
void TestWorldYawMatchesLocalWhenLevel() {
    const Mat3 local = gc_ht::idtech::RotateBasisLocal(Level(), 20.0f * kDeg, 0.0f, 0.0f);
    const Mat3 world = gc_ht::idtech::RotateBasisWorldYaw(Level(), 20.0f * kDeg, 0.0f, 0.0f);
    for (int i = 0; i < 9; ++i) {
...
void TestWorldYawDiffersWhenLookingDown() {
    ...
    const Mat3 world = gc_ht::idtech::RotateBasisWorldYaw(down, 20.0f * kDeg, 0.0f, 0.0f);
    ...
    const Mat3 local = gc_ht::idtech::RotateBasisLocal(down, 20.0f * kDeg, 0.0f, 0.0f);
...
void TestUnitScale() {
    CheckNear(gc_ht::idtech::kUnitsPerMetre, 1.0f, 1e-6f,
              "one metre is one world unit - this engine is metric, unlike id Tech 5");
}
...
void TestTranslationFollowsTheBasisRows() {
    const gc_ht::idtech::Vec3 origin{10.0f, 20.0f, 30.0f};
    const gc_ht::idtech::Vec3 out =
        gc_ht::idtech::TranslateAlongBasis(origin, Level(), 4.0f, 2.0f, 1.0f);
    ...
    const Mat3 turned = gc_ht::idtech::RotateBasisLocal(Level(), 90.0f * kDeg, 0.0f, 0.0f);
    const gc_ht::idtech::Vec3 leaned =
        gc_ht::idtech::TranslateAlongBasis(origin, turned, 4.0f, 0.0f, 0.0f);
...
void TestOrthonormalGuardAcceptsAndRefuses() {
    Check(gc_ht::idtech::IsOrthonormal(Level()), "the guard passes a level basis");
    Check(gc_ht::idtech::IsOrthonormal(
              gc_ht::idtech::RotateBasisLocal(Level(), 15.0f * kDeg, 25.0f * kDeg, 35.0f * kDeg)),
          "the guard passes a rotated basis");

    Mat3 scaled = Level();
    scaled.m[0] = 2.0f;
    Check(!gc_ht::idtech::IsOrthonormal(scaled), "the guard refuses a row that is not unit length");

    // One case per perpendicularity clause, each rotated IN PLANE so every row
    // stays exactly unit length. That is what makes them reach those clauses at
    // all: the case here before set m[3] to 0.5 and left m[4] at 1, giving row 1
    // a self-dot of 1.25, so the guard refused it on the unit-length branch and
    // all three Dot(Row(a), Row(b)) clauses had no coverage whatsoever. Deleting
    // them left this suite green while the hook began writing a sheared basis
    // into the engine.
    //
    // Three cases rather than one because the clauses are independent: a single
    // row0/row1 skew leaves Dot(Row0,Row2) and Dot(Row1,Row2) at zero, so two of
    // the three stayed uncovered.
    const float lean = 11.0f * kDeg;
    struct Skew {
        int a, b;
        const char* what;
    };
    const Skew skews[] = {
        {3, 4, "the guard refuses row 1 leaning into row 0"},
        {6, 8, "the guard refuses row 2 leaning into row 0"},
        {7, 8, "the guard refuses row 2 leaning into row 1"},
    };
    for (const Skew& skew : skews) {
        Mat3 skewed = Level();
        skewed.m[skew.a] = std::sin(lean);
        skewed.m[skew.b] = std::cos(lean);
        const float length =
            std::sqrt(skewed.m[skew.a] * skewed.m[skew.a] + skewed.m[skew.b] * skewed.m[skew.b]);
        CheckNear(length, 1.0f, 1e-6f,
                  "the skewed row is still unit length, so only perpendicularity can refuse it");
        Check(!gc_ht::idtech::IsOrthonormal(skewed), skew.what);
    }

    Mat3 broken = Level();
    broken.m[4] = std::numeric_limits<float>::quiet_NaN();
    Check(!gc_ht::idtech::IsOrthonormal(broken), "the guard refuses a basis carrying NaN");
}
