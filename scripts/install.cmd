@echo off
:: ============================================
:: Indiana Jones and the Great Circle Head Tracking - Install
:: ============================================
...
:: --- CONFIG BLOCK ---
set "GAME_ID=indiana-jones-and-the-great-circle"
set "MOD_DISPLAY_NAME=Indiana Jones and the Great Circle Head Tracking"
set "MOD_DLLS=GreatCircleHeadTracking.asi"
set "MOD_INTERNAL_NAME=GreatCircleHeadTracking"
...
set "MOD_SEED_FILES="
:: Version of the vendored Ultimate ASI Loader, recorded in the state file this
:: standalone install writes. The launcher never reads it - this package is
:: delivery_mode "manifest" and the launcher does not run install.cmd. Leave
:: empty to omit the field. Stamped by `pixi run update-deps`.
set "ASI_LOADER_VERSION=9.7.4"
:: Post-install help text. `&echo ` starts each further line.
set "MOD_CONTROLS=Controls (nav cluster / chord):&echo   End      / Ctrl+Shift+Y  Toggle tracking&echo   PageUp   / Ctrl+Shift+G  Cycle tracking mode: full, rotation only, position only&echo   PageDown / Ctrl+Shift+H  Toggle yaw mode: world-locked or camera-local&echo.&echo Recentring is done in your tracker app, not in the game."
:: --- END CONFIG BLOCK ---
